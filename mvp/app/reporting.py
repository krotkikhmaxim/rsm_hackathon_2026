...
def generate_markdown_report(prediction_result):
    ...
    for threat_row in prediction_result["allthreats"]:
        risk_level = "Высокий" if threat_row["probability"] >= 0.7 else "Средний" if threat_row["probability"] >= 0.4 else "Низкий"
        report_lines.append(f"- **{threat_row['threatname']}** — {threat_row['probability']:.1%} ({risk_level} риск)")
